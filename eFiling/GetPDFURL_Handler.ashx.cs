...
{
    /// <summary>
    /// GetPDFURLHandler 的摘要描述
    /// </summary>
    public class GetPDFURL_Handler : IHttpHandler
    {
        #region MyUtility
        /// <summary>
        /// 
        ...
        {
            this.MyUtility.InitLog("GetPDFURL");
            ...
            PDF_URL_CLASS.PDF_URL_ITEM pdf_info_item = new PDF_URL_CLASS.PDF_URL_ITEM();
            try
            {
                #region 取得參數
                ...
                if (String.IsNullOrEmpty(session_key))
                {
                    ...
                    this.MyUtility.DBLog(context, SysCode.B001, "GetPDFURL", string.Empty, USERID, session_info = string.Format("{0}_{1}_{2}_{3}_{4}_{5}", year_and_month, CHANNEL_CODE, TXN_TYPE, TXN_ID, REPORT_SERIAL_NO, GUID));
                ...
                else
                {
                    ...
                    this.MyUtility.DBLog(context, SysCode.B001, "GetPDFURL", string.Empty, USERID, session_info = string.Format("{0}_{1}", year_and_month, session_key));
                ...
                this.MyUtility.DBLog(context, SysCode.A001, "GetPDFURL", session_key = dt.Rows[0]["SESSION_KEY"].ToString(), USERID, string.Empty);
                ...
                if (dr_pdf.Length > 0)
                {
                    ...
                    file_seq = Convert.ToInt16(dr_pdf[0]["FILE_SEQ"].ToString());
                    file_type = Convert.ToInt16(dr_pdf[0]["FILE_TYPE"].ToString());
                ...
                string image_url = context.Request.Url.AbsoluteUri.Replace("GetPDFURL", "");
                ...
                pdf_info_item.URL = image_url.EncryptBase64();

                this.MyUtility.DBLog(context, SysCode.B000, "GetPDFURL", session_key, USERID, session_info);
            }
            catch (System.Exception ex)
            {
                ...
                this.MyUtility.DBLog(context, code, "GetPDFURL", session_key, USERID, ex.Message);

                this.MyUtility.SendEMail(context, "GetPDFURL", session_key, code);
            }
            finally
            {
                json = JsonConvert.SerializeObject(new PDF_URL_RESPOSE()
                {
                    CHANGINGTEC = new PDF_URL_SYSTEM_CLASS()
                    {
                        SYSTEM = new PDF_URL_SYSTEM_INFO_CLASS()
                        {
                            ...
                            CASE_INFO = new PDF_URL_CLASS() { PDF_ITEM = pdf_info_item }
                        }
                    }
                });
                dt = null;
